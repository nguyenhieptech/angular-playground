...
export class ThemeService {
  ...
  private readonly _query = inject(MediaMatcher).matchMedia(
  ...
  constructor() {
    ...
    this._destroyRef.onDestroy(() =>
      this._query.removeEventListener("change", handleChange)
    );
    ...
    effect(() => {
      ...
      const oldLayout =
        this._document.documentElement.className.match(/layout-(\w+)/)?.[1];
