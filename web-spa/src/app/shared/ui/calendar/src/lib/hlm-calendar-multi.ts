...
@Component({
  selector: "[hlmCalendarMulti],hlm-calendar-multi",
  imports: [
    BrnCalendarImports,
    NgIcon,
    HlmIcon,
          ...
          <tbody role="rowgroup">
            <tr *brnCalendarWeek="let week" class="mt-2 flex w-full">
              @for (date of week; track _dateAdapter.getTime(date)) {
                <td
                  brnCalendarCell
                  class="relative h-8 w-8 p-0 text-center text-sm focus-within:relative focus-within:z-20 data-[selected]:bg-accent first:data-[selected]:rounded-l-md last:data-[selected]:rounded-r-md data-[selected]:data-[outside]:bg-accent/50 [&:has([aria-selected].day-range-end)]:rounded-r-md"
                >
